...
pub struct Row<'a> {
    data: &'a [u8],
    _fields: &'a Arc<[SortField]>, // currently not used.
}
...
mod tests {
    ...
    fn test_variable_width() {
        let col = Utf8Array::<i32>::from([Some("hello"), Some("he"), None, Some("foo"), Some("")])
            .to_boxed();
    ...
    fn fuzz_test() {
        for _ in 0..100 {
            ...
            let comparators = arrays
                .iter()
                .zip(options.iter())
                .map(|(a, o)| build_compare(&**a, o.clone()).unwrap())
                .collect::<Vec<_>>();

...
fn take_no_validity<O: Offset, I: Offset>(
    offsets: &[O],
    values: &[u8],
    indices: &[I],
...
fn take_values_validity<O: Offset, I: Offset>(
    values: &Utf8Array<O>,
    indices: &[I],
) -> Result<(Buffer<O>, Buffer<u8>, Option<Bitmap>)> {
    let mut length = O::default();
...
fn take_indices_validity<O: Offset, I: Offset>(
    offsets: &[O],
    values: &[u8],
...
fn take_values_indices_validity<O: Offset, I: Offset>(
    values: &Utf8Array<O>,
    indices: &PrimitiveArray<I>,
) -> Result<(Buffer<O>, Buffer<u8>, Option<Bitmap>)> {
    let mut length = O::default();
...
pub fn take<O: Offset, I: Offset>(
    ...
    let (offsets, values, validity) = match (values_has_validity, indices_has_validity) {
        ...
        (true, false) => take_values_validity::<O, I>(values, indices.values())?,
        (false, true) => take_indices_validity::<O, I>(values.offsets(), values.values(), indices)?,
        (true, true) => take_values_indices_validity::<O, I>(values, indices)?,
    };

        ...
        self.columns
            ...
            .all(|((col, arr), field)| {
                ...
                match field.data_type() {
                    DataType::Null => {
                        let arr: &NullArray = arr.as_any().downcast_ref::<NullArray>().unwrap();
                            ...
                            .map(|v| {
                                match v {
                                    ...
                                    Value::Object(v) => {
                                        ...
                                        let days: i32 =
                                            v.get("days").unwrap().as_i64().unwrap() as i32;
                                        let milliseconds: i32 =
                                            v.get("milliseconds").unwrap().as_i64().unwrap() as i32;
                    ...
                    DataType::Binary => {
                        let arr = arr.as_any().downcast_ref::<BinaryArray<i32>>().unwrap();
                        arr.equals_json(&json_array.iter().collect::<Vec<&Value>>()[..])
                    }
    ...
    pub fn from_batch(batch: &RecordBatch) -> ArrowJsonBatch {
        ...
        for (col, field) in batch.columns().iter().zip(batch.schema().fields.iter()) {
            let json_col = match field.data_type() {
                DataType::Int8 => {
                    let col = col.as_any().downcast_ref::<PrimitiveArray<i8>>().unwrap();
            ...
            let converted_col: Vec<Value> = col
                ...
                .map(|v| {
                    Value::Number(match v {
                        ...
                        Value::String(string) => Number::from(
                            string
                                .parse::<i64>()
...
fn json_from_list_col(col: &ArrowJsonColumn, data_type: &DataType) -> Vec<Value> {
    ...
    let offsets: Vec<usize> = col
        ...
        .map(|o| match o {
            ...
            _ => panic!("Offsets should be numbers or strings that are convertible to numbers"),
        })
        .collect();
...
mod tests {
    ...
    fn test_schema_equality() {
        ...
        let schema = Schema::new(vec![
            ...
            Field::new(
                "c4",
                DataType::List(Box::new(Field::new("custom_item", DataType::Int32, false))),
                true,
            ),
        ]);
    ...
    fn test_arrow_data_equality() {
        ...
        let schema = Schema::new(vec![
            {
                let mut f = Field::new("bools-with-metadata-map", DataType::Boolean, true);
                f.set_metadata(Some(
            ...
            {
                let mut f = Field::new("bools-with-metadata-vec", DataType::Boolean, true);
                f.set_metadata(Some(
        ...
        let ts_secs =
            TimestampSecondArray::from_opt_vec(vec![None, Some(193438817552), None], None);
        let ts_millis = TimestampMillisecondArray::from_opt_vec(
            vec![None, Some(38606916383008), Some(58113709376587)],
            None,
        );
        let ts_micros = TimestampMicrosecondArray::from_opt_vec(vec![None, None, None], None);
        let ts_nanos = TimestampNanosecondArray::from_opt_vec(
            vec![None, None, Some(-6473623571954960143)],
            None,
        );
        let ts_secs_tz =
            TimestampSecondArray::from_opt_vec(vec![None, Some(193438817552), None], secs_tz);

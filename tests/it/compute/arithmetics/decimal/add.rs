...
fn test_add_panic() {
    ...
    let b = PrimitiveArray::from([Some(1i128)]).to(DataType::Decimal(DecimalType::Int128, 5, 2));
    let _ = add(&a, &b);
}
...
fn test_add_checked_overflow() {
    let a = PrimitiveArray::from([Some(1i128), Some(99999i128)]).to(DataType::Decimal(
        DecimalType::Int128,
        5,
        2,
    ));
    let b = PrimitiveArray::from([Some(1i128), Some(1i128)]).to(DataType::Decimal(
        DecimalType::Int128,
        5,
        2,
    ));
    let result = checked_add(&a, &b);
    let expected =
        PrimitiveArray::from([Some(2i128), None]).to(DataType::Decimal(DecimalType::Int128, 5, 2));
    assert_eq!(result, expected);
...
fn test_add_adaptive() {
    ...
    // 11122.2211 -> 9, 4
    let a =
        PrimitiveArray::from([Some(11_1111i128)]).to(DataType::Decimal(DecimalType::Int128, 6, 4));
    let b =
        PrimitiveArray::from([Some(11111_11i128)]).to(DataType::Decimal(DecimalType::Int128, 7, 2));
    let result = adaptive_add(&a, &b).unwrap();

    let expected = PrimitiveArray::from([Some(11122_2211i128)]).to(DataType::Decimal(
        DecimalType::Int128,
        9,
        4,
    ));
    ...
    // -----------------
    // 11111.1111 -> 9, 4
    let a = PrimitiveArray::from([Some(1111i128)]).to(DataType::Decimal(DecimalType::Int128, 5, 4));
    let b =
        PrimitiveArray::from([Some(11111_0i128)]).to(DataType::Decimal(DecimalType::Int128, 6, 1));
    let result = adaptive_add(&a, &b).unwrap();

    let expected = PrimitiveArray::from([Some(11111_1111i128)]).to(DataType::Decimal(
        DecimalType::Int128,
        9,
        4,
    ));
    ...
    let a =
        PrimitiveArray::from([Some(11111_11i128)]).to(DataType::Decimal(DecimalType::Int128, 7, 2));
    let b = PrimitiveArray::from([Some(11111_111i128)]).to(DataType::Decimal(
        DecimalType::Int128,
        8,
        3,
    ));
    ...
    let expected = PrimitiveArray::from([Some(22222_221i128)]).to(DataType::Decimal(
        DecimalType::Int128,
        8,
        3,
    ));
    ...
    // 100.0000 -> 7, 4
    let a =
        PrimitiveArray::from([Some(99_9999i128)]).to(DataType::Decimal(DecimalType::Int128, 6, 4));
    let b =
        PrimitiveArray::from([Some(00_0001i128)]).to(DataType::Decimal(DecimalType::Int128, 6, 4));
    let result = adaptive_add(&a, &b).unwrap();

    let expected =
        PrimitiveArray::from([Some(100_0000i128)]).to(DataType::Decimal(DecimalType::Int128, 7, 4));

    assert_eq!(result, expected);
    assert_eq!(
        result.data_type(),
        &DataType::Decimal(DecimalType::Int128, 7, 4)
    );
}
